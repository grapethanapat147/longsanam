...
- [ ] A pay-later seat with no check-in and no cancel is charged −10 exactly
      once, with a `credit_events` row, and never on a cancelled session.
- [ ] A paid no-show is charged nothing, pinned.
- [ ] Chase reminders include the check-in time when one exists.
- [ ] `mark_no_shows()` is idempotent under the five-minute sweep, pinned.
