...
- [ ] Organizer can remove a guest while the session is still open.
- [ ] `list_chaseable_participants()` never returns a guest, pinned.
- [ ] `mark_no_shows()` never charges a guest, pinned.
- [ ] `join_session`, `payForSlotAction`, `grant_pay_later` and `record_chase`
      refuse a guest row with a clear reason.
- [ ] A guest counts in LSN-0021's settlement denominator, pinned.
- [ ] Every existing pgTAP assertion still passes with `user_id` nullable.
