    ...
    <AppShell>
      <article>
        <header className="mb-6">
          ...
          <h1 className="mt-2.5 text-2xl font-semibold tracking-tight text-ink-900 sm:text-3xl">
            {session.title}
          </h1>
          <p className="mt-1.5 text-sm text-ink-600">{sessionStatusHint[session.status]}</p>
        </header>
          ...
          <div className="order-2 space-y-5 lg:order-none">
            ...
            <Card className="px-5 py-4">
              <dl className="grid gap-3 sm:grid-cols-2">
                ...
                />
                <Detail
                  label={t.session.organizer}
                  value={session.organizer?.display_name ?? '—'}
                />
                <Detail
              ...
              {session.description ? (
                <p className="mt-4 whitespace-pre-line border-t border-ink-200 pt-4 text-sm text-ink-700">
                  {session.description}
                </p>
              ) : null}
            </Card>
            ...
            <Card className="px-5 py-4">
              <h2 className="font-semibold text-ink-900">{t.session.participants}</h2>
              <div className="mt-3">
                <Progress
                  value={counts.paid}
              ...
              <p className="mt-3 text-sm text-ink-600">
                ...
                {cheapestCourt !== null ? (
                  <>
                    ...
                    {formatThb(costPerPersonThb(cheapestCourt, session.min_players))}{' '}
                    ต่อคนเมื่อครบขั้นต่ำ)
                  </>
                ) : null}
              </p>
            </Card>
            ...
            <Card className="px-5 py-4">
              <h2 className="font-semibold text-ink-900">{t.session.venuePreferences}</h2>
              <p className="mt-1 text-sm text-ink-600">{t.session.fallbackNote}</p>
