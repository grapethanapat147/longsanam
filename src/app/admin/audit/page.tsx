...
export default async function AdminAuditPage({
  ...
  let query = admin
    .from('audit_logs')
    .select(
      'id, actor_id, entity_type, entity_id, session_id, action, from_state, to_state, metadata, created_at',
    )
            ...
            {rows.map((row) => (
              <tr key={row.id}>
                ...
                <td className="px-4 py-3">
                  <span className="font-mono text-xs text-ink-900">{row.action}</span>
...
function FilterLink({ href, active, label }: { href: string; active: boolean; label: string }) {
  return (
    <Link
      ...
      className={
        active
          ? 'rounded-full bg-brand-600 px-3 py-1.5 text-xs font-semibold text-on-brand focus-ring'
